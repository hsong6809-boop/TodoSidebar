...
- ⚪ **第三形态「悬浮球」**：侧边栏/完整窗口之外新增悬浮球形态——胶囊（进度环+轮换信息）与圆形（进度环+中心完成数）两种形状；番茄进行中自动插队显示倒计时
- 🔄 **信息轮换**：接下来待办 / 今日进度 / 输入字数 / 连击四类信息按间隔轮换（默认仅「接下来」，可在设置勾选，3~30 秒可调）
- 🖱️ **零打扰交互**：单击手动翻页、双击展开完整窗口、拖动换位（位置记忆）、右键菜单（切形态/锁位置/设置）；WM_MOUSEACTIVATE 返回 MA_NOACTIVATE，点击拖动全程不抢焦点
- 🌙 **闲置淡化**：鼠标离开 20 秒自动降到 35% 透明度，靠近恢复；3 秒周期重申置顶（与侧边栏同策略）
- ⌨️ **Ctrl+Alt+T 三态循环**：侧边栏 → 完整 → 悬浮球循环切换；侧边栏/完整窗口头部新增悬浮球按钮；启动形态可在设置选择（默认完整窗口）
- ⚙️ **设置 → 显示形态**：启动形态 / 悬浮球形状 / 轮换内容与间隔 / 闲置淡化 / 锁定位置，全部即时生效
- 🔧 **发布前审查修复**：模式切换的窗口构造函数异常改为统一兜底（失败不再破坏当前形态）；设置页回填改为统一静默、不再连锁写库；悬浮球双击展开不再先闪一页；悬浮球右键菜单补「退出应用」；去掉悬浮球每秒重复刷新

### v5.6.3 (2026-09-01)

- 🛠️ **修复完整模式统计页缺输入统计卡**：完整模式「统计」页此前仅「今日」卡有一行输入小字，现补齐独立输入统计卡——今日/本周/本月/今年四档切换 + 估算字数/击键/日均字数/活跃天数 + 逐日（今年为月度）趋势图
